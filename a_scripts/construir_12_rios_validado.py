...
"""
================================================================================
SCRIPT MESTRE: GERAÇÃO, VALIDAÇÃO INDIVIDUAL E UNIÃO DOS 12 RIOS DO VALE
Diretório do Script : a_scripts/construir_12_rios_validado.py
Destinos            : modelos/_anti/<nome_rio> (Individuais) e _anti/taha_ai (Mestre)
================================================================================
"""
import os
import re
...
DIR_INDIVIDUAIS = os.path.join(DIR_RAIZ, "modelos", "_anti")
...
# ==============================================================================
# 1. CARREGAMENTO E UNIFICAÇÃO DOS 12 EIXOS FLUVIAIS
# ==============================================================================
def obter_12_eixos_unificados():
    print("  [1/4] Carregando e unificando os 12 eixos fluviais contínuos...")
    ...
    for feat in d_eixo["features"]:
        ...
        if r not in eixos_por_rio:
            eixos_por_rio[r] = []
        eixos_por_rio[r].append((reach, LineString(coords)))

    eixos_unificados = {}
    for r, lista in eixos_por_rio.items():
        lista_ord = sorted(lista, key=lambda x: x[0])
        todas_coords = []
        for reach_name, ls in lista_ord:
            c = list(ls.coords)
            if not todas_coords:
                todas_coords.extend(c)
            else:
                todas_coords.extend(c[1:])
        eixos_unificados[r] = LineString(todas_coords)

    # Para o Rio Itajaí-Mirim: Fundir com o Canal Retificado de Itajaí
    eixo_mirim_nat = eixos_unificados["Itajai_Mirim"]
    ...
    eixo_mirim_final = LineString(list(eixo_mirim_mont.coords) + list(canal_merged.coords))
    eixos_unificados["Itajai_Mirim"] = eixo_mirim_final

    for r, ls in eixos_unificados.items():
        print(f"    - {r:18s}: {ls.length/1000:6.2f} km | {len(ls.coords)} vertices")
    return eixos_unificados


# ==============================================================================
# 2. GERAÇÃO DE CUTLINES E DESENTRELAÇAMENTO DE EDGE LINES / BANK LINES
# ==============================================================================
def gerar_cutlines_rio(eixo, nome_rio, dx=200.0):
    ...
    w_min, w_max, w_corte = larguras_dict.get(nome_rio, (30.0, 50.0, 180.0))
    ...
    # Algoritmo de desentrelaçamento iterativo de Edge Lines
...
# ==============================================================================
# 3. AMOSTRAGEM DO RELEVO SIG-SC E ESCULTURA DA CALHA ATIVA
# ==============================================================================
def amostrar_e_esculpir_rio(secoes, mosaico, nome_rio):
    ...
    # Cotas de referência parametrizadas por fração do rio [0.0 a 1.0]
    cotas_frac_dict = {
        "Itajai_Acu": [(0.0, 350.0), (0.22, 110.0), (0.54, 35.0), (0.80, 9.0), (1.0, -2.5)],
        "Itajai_Norte": [(0.0, 480.0), (0.38, 260.0), (0.80, 145.0), (1.0, 112.0)],
        "Itajai_Oeste": [(0.0, 520.0), (0.50, 420.0), (0.85, 365.0), (1.0, 350.0)],
        "Itajai_Sul": [(0.0, 490.0), (0.45, 420.0), (0.85, 370.0), (1.0, 350.0)],
        "Itajai_Mirim": [(0.0, 275.0), (0.30, 115.0), (0.65, 12.0), (1.0, -2.68)],
        "Rio_Benedito": [(0.0, 220.0), (0.65, 85.0), (1.0, 35.0)],
        "Rio_dos_Cedros": [(0.0, 180.0), (1.0, 85.0)],
        "Rio_Trombudo": [(0.0, 460.0), (1.0, 360.0)],
        "Rio_Iraputa": [(0.0, 510.0), (1.0, 260.0)],
        "Rio_Taio": [(0.0, 530.0), (1.0, 420.0)],
        "Rio_das_Pombas": [(0.0, 450.0), (1.0, 370.0)],
        "Rio_do_Testo": [(0.0, 120.0), (1.0, 20.0)],
    }
    ctrl = cotas_frac_dict.get(nome_rio, [(0.0, 300.0), (1.0, 10.0)])
    s_ctrl = np.array([p[0] * s_max for p in ctrl])
...
# ==============================================================================
# 4. GERAÇÃO E VALIDAÇÃO INDIVIDUAL DE CADA RIO (PASSO 3 DO USUÁRIO)
# ==============================================================================
def gerar_e_validar_rio_individual(nome_rio, eixo, secoes):
    pasta_rio = pathlib.Path(DIR_INDIVIDUAIS) / nome_rio.lower()
    pasta_rio.mkdir(parents=True, exist_ok=True)
    nome_proj = nome_rio.lower()
    
    # 1. Geometria Individual (.g01)
    pts_xy = list(eixo.coords)
    g01_txt = f"Geom Title={nome_rio} - Validado\nProgram Version=7.01\nSpatial Reference System={WKT_CRS}\n\n"
    g01_txt += f"River Reach={nome_rio:<16s},R1              \n"
    g01_txt += format_reach_xy(pts_xy) + "\n"
    ...
    escrever_texto_dos(str(pasta_rio / f"{nome_proj}.g01"), g01_txt)
...
"""
    escrever_texto_dos(str(pasta_rio / f"{nome_proj}.prj"), prj_txt)
...
Run RASMapper=-1
"""
    escrever_texto_dos(str(pasta_rio / f"{nome_proj}.p01"), p01_txt)
    
    # 4. Condições de Contorno (.u01)
    rs_mont = secoes[0]["rs"]
    rs_jus = secoes[-1]["rs"]
    u01_txt = f"""Flow Title={nome_proj}
Program Version=7.01
Use Restart= 0 

Initial RS={nome_rio:<16s},R1              ,{rs_mont:.2f},  50

Boundary Location={nome_rio:<16s},R1              ,{rs_mont:.2f},        ,                ,                
Interval=1HOUR
Flow Hydrograph= 97 
...
Boundary Location={nome_rio:<16s},R1              ,{rs_jus:.2f},        ,                ,                
Friction Slope= 0.0005
"""
    escrever_texto_dos(str(pasta_rio / f"{nome_proj}.u01"), u01_txt)
    
    # 5. Projeção e RAS Mapper (.prj, .rasmap)
    with open(pasta_rio / "SIRGAS2000_UTM22S.prj", "w", encoding="utf-8") as f:
        f.write(WKT_CRS)
  ...
  <Geometries Checked="True" Expanded="True">
    <Layer Name="{nome_rio} - Validado" Type="RASGeometry" Checked="True" Expanded="True" Filename=".\\{nome_proj}.g01.hdf">
...
"""
    escrever_texto_dos(str(pasta_rio / f"{nome_proj}.rasmap"), rasmap_xml)
    
    terrain_raiz = os.path.join(DIR_RAIZ, "modelo", "Terrain")
    if os.path.exists(terrain_raiz) and not (pasta_rio / "Terrain").exists():
        shutil.copytree(terrain_raiz, pasta_rio / "Terrain")
        
    pts_esq = [Point(s["cut"].coords[0]) for s in secoes]
    pts_dir = [Point(s["cut"].coords[1]) for s in secoes]
    edge_esq = LineString(pts_esq)
    edge_dir = LineString(pts_dir)
    print(f"    [Validação Topológica] {nome_rio:18s}: {len(secoes):4d} seções | Edge Esq: {edge_esq.is_simple} | Edge Dir: {edge_dir.is_simple}")
    return str(pasta_rio / f"{nome_proj}.prj")


# ==============================================================================
# 5. MONTAGEM E SIMULAÇÃO DO MODELO MESTRE DA BACIA (12 RIOS JUNTOS)
# ==============================================================================
def montar_modelo_mestre_12_rios(dados_rios):
    print("\n" + "=" * 72)
    print("MONTANDO MODELO MESTRE UNIFICADO (12 RIOS) EM _anti/taha_ai...")
    print("=" * 72)
    ...
    for nome_rio, eixo, secoes in dados_rios:
        total_secoes += len(secoes)
        pts_xy = list(eixo.coords)
        reach_txt = f"River Reach={nome_rio:<16s},R1              \n"
    ...
    print(f"  [1/4] Geometria unificada gravada ({len(info_rios)} rios, {total_secoes} seções).")
...
"""
    ...
    print(f"  [4/4] Arquivos HEC-RAS mestre e RASMapper configurados.")
...
def main():
    print("=" * 72)
    print("PIPELINE COMPLETO DE GERAÇÃO E VALIDAÇÃO DOS 12 RIOS DO VALE")
    print("=" * 72)
    
    eixos = obter_12_eixos_unificados()
    mosaico = MosaicoSigsc()
    
    dados_rios = []
    for nome_rio, eixo in eixos.items():
        print(f"\n--> Modelando {nome_rio} ({eixo.length/1000:.2f} km)...")
        secoes = gerar_cutlines_rio(eixo, nome_rio, dx=200.0)
        secoes = amostrar_e_esculpir_rio(secoes, mosaico, nome_rio)
        
        gerar_e_validar_rio_individual(nome_rio, eixo, secoes)
        dados_rios.append((nome_rio, eixo, secoes))
        
    prj_mestre = montar_modelo_mestre_12_rios(dados_rios)
    executar_e_auditar(prj_mestre)
