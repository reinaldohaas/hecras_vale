...
MEIA_MAX = 220.0      # m; teto da meia-largura
...
def main():
    ...
    LEG_OVERBANK = 80.0     # m para fora de cada Bank Sta nas secoes levantadas
    ...
    def adotar(k, s, p, fundo_max=None):
        ...
        for j in np.flatnonzero(_dono == k) if len(_dono) else []:
            ...
            # A secao levantada original vem com ate 4 km de planicie. No RAS
            # 1D isto cruza banklines de outros bracos do meandro e ainda
            # deixa o modelo pesado. Mantem-se a calha medida e uma faixa
            # controlada de overbank, com Bank Sta convertida para a nova
            # origem de estaca.
            rec0 = max(float(stt[0]), lb_ - LEG_OVERBANK)
            rec1 = min(float(stt[-1]), rb_ + LEG_OVERBANK)
            i0r = max(0, int(np.searchsorted(stt, rec0, side="left")) - 1)
            i1r = min(len(stt), int(np.searchsorted(stt, rec1, side="right")) + 1)
            if i1r - i0r >= 8:
                st0, st1 = float(stt[i0r]), float(stt[i1r - 1])
                span = max(float(stt[-1] - stt[0]), 1e-9)
                frac0 = (st0 - float(stt[0])) / span
                frac1 = (st1 - float(stt[0])) / span
                _A2 = _A + frac0 * (_B - _A)
                _B2 = _A + frac1 * (_B - _A)
                stt = stt[i0r:i1r] - st0
                zz_ = zz_[i0r:i1r]
    ...
    # ---- VAO GRAVE: se os filtros deixam um buraco maior que o contrato, poe
    # uma secao intermediaria estreita, interpolada das duas vizinhas ja
    # aceitas. Ela nao tenta carregar a planicie; serve para dar ao RAS uma
    # transicao geometrica 1D sem abrir edge line nem violar espacamento.
    novas_gap = []
    for a_, b_ in zip(secoes[:-1], secoes[1:]):
        if b_["s"] - a_["s"] <= 1600.0:
            continue
        s_ = 0.5 * (a_["s"] + b_["s"])
        A0, A1 = np.asarray(a_["cut"][0], float), np.asarray(a_["cut"][1], float)
        B0, B1 = np.asarray(b_["cut"][0], float), np.asarray(b_["cut"][1], float)
        C0m, C1m = 0.5 * (A0 + B0), 0.5 * (A1 + B1)
        largura = float(np.hypot(*(C1m - C0m)))
        q0_ = np.array(eixo.interpolate(max(s_ - JANELA, 0.0)).coords[0])
        q1_ = np.array(eixo.interpolate(min(s_ + JANELA, L)).coords[0])
        t_ = q1_ - q0_
        t_ = t_ / max(float(np.hypot(*t_)), 1e-9)
        nloc_ = np.array([-t_[1], t_[0]])
        centro_ = np.array(eixo.interpolate(s_).coords[0])
        C0, C1 = centro_ + 0.5 * largura * nloc_, centro_ - 0.5 * largura * nloc_
        n_ = max(9, int(round(largura / PASSO)) + 1)
        sta_ = np.linspace(0.0, largura, n_)
        f_ = np.linspace(0.0, 1.0, n_)
        fa_ = np.linspace(0.0, 1.0, len(a_["sta"]))
        fb_ = np.linspace(0.0, 1.0, len(b_["sta"]))
        z_ = 0.5 * (np.interp(f_, fa_, a_["z"]) +
                    np.interp(f_, fb_, b_["z"]))
        la_ = max(float(a_["sta"][-1] - a_["sta"][0]), 1e-9)
        lbw_ = max(float(b_["sta"][-1] - b_["sta"][0]), 1e-9)
        lb_ = largura * 0.5 * (float(a_["lb"]) / la_ +
                               float(b_["lb"]) / lbw_)
        rb_ = largura * 0.5 * (float(a_["rb"]) / la_ +
                               float(b_["rb"]) / lbw_)
        eixo_sta_ = 0.5 * largura
        cand_ = sta_[sta_ <= eixo_sta_ - 30.0]
        lb_ = float(cand_.max()) if len(cand_) else float(sta_[0])
        cand_ = sta_[sta_ >= eixo_sta_ + 30.0]
        rb_ = float(cand_.min()) if len(cand_) else float(sta_[-1])
        if not (lb_ < eixo_sta_ < rb_):
            j_ = int(np.argmin(np.abs(sta_ - eixo_sta_)))
            lb_ = float(sta_[max(j_ - 1, 0)])
            rb_ = float(sta_[min(j_ + 1, len(sta_) - 1)])
        novas_gap.append({"s": s_, "rs": round(float(L - s_), 2),
                          "sta": np.round(sta_, 2), "z": np.round(z_, 2),
                          "lb": round(float(lb_), 2), "rb": round(float(rb_), 2),
                          "cut": (C0, C1), "zt": float(np.min(z_))})
    if novas_gap:
        secoes = sorted(secoes + novas_gap, key=lambda q: q["s"])
        print(f"   vao grave: {len(novas_gap)} secao(oes) intermediaria(s) "
              "interpolada(s) entre vizinhas aceitas")
