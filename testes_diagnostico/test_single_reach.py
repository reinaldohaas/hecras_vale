...
dem_sampler = gerar_geometria_hecras.DemSampler(r'C:\Users\haas\github\hecras_vale\dem_itajai.tif')
to_utm = Transformer.from_crs(4326, 31982, always_xy=True)

with open(r'C:\Users\haas\github\hecras_vale\rios_itajai.geojson', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
for feat in data['features']:
    ...
    if gtype == 'LineString':
        lines.append(LineString([to_utm.transform(pt[0], pt[1]) for pt in coords]))
    elif gtype == 'MultiLineString':
        for seg in coords:
            lines.append(LineString([to_utm.transform(pt[0], pt[1]) for pt in seg]))
...
print('Comprimento do canal principal UTM:', main_line.length, 'm')

dists = np.linspace(0, main_line.length, 60)
xs_list = []
for d in dists:
...
print(f'Total de seções amostradas no canal principal: {len(xs_list)}')

prj_name = 'Itajai_Calha_Principal'
g01_file = r'C:\Users\haas\github\hecras_vale\Itajai_Calha_Principal.g01'
prj_file = r'C:\Users\haas\github\hecras_vale\Itajai_Calha_Principal.prj'
u01_file = r'C:\Users\haas\github\hecras_vale\Itajai_Calha_Principal.u01'
p01_file = r'C:\Users\haas\github\hecras_vale\Itajai_Calha_Principal.p01'

with open(g01_file, 'w', encoding='ascii') as f:
...
with open(prj_file, 'w', encoding='ascii') as f:
...
with open(u01_file, 'w', encoding='ascii') as f:
    f.write('Flow Title=Cheia_Sintetica\n')
    f.write('Program Version=6.10\n\n')
    hidro = gerar_geometria_hecras.gerar_hidrograma(1500.0)
...
with open(p01_file, 'w', encoding='ascii') as f:
...
print('Arquivos HEC-RAS de calha única gerados com sucesso!')

ver = r'C:\Program Files (x86)\HEC\HEC-RAS\7.0.1\Ras.exe'
print('Testando ras-commander com calha contínua...')
init_ras_project(prj_file, ver)
