...
- Cheia Secular de 1983 (Julho/1983)
- Desastre de 2008 (Novembro/2008)
- Cheia de 2011 (Setembro/2011)
- Cheia Recente de 2023 (Outubro-Novembro/2023)
"""
...
class HistoricalEventsLoader:
    ...
    EVENTS_METADATA = {
        ...
        '2008': {
            ...
            'duration_hours': 72,
            'p5_antecedent_mm': 78.0, # AMC III (Extrema Saturação no Médio e Baixo Vale)
            'blumenau_stage_m': 11.52,
            'blumenau_peak_obs_m3s': 4200.0,
            'rio_do_sul_peak_obs_m3s': 1950.0,
            'brusque_peak_obs_m3s': 1650.0,
            'itajai_peak_obs_m3s': 5700.0,
            'dam_operating_status': 'Oeste e Sul Operando / Grande impacto em encostas e Baixo Vale'
        },
        '2011': {
            ...
            'dam_operating_status': 'Operação Total (Oeste, Sul e Norte)'
        },
        '2023': {
    ...
    def get_event_data(cls, event_id: str) -> Dict[str, Any]:
        """
        Retorna chuva horária distribuída e hidrogramas observados para o evento.
        """
        ev_id = str(event_id)
        ...
        # Gerar série horária realista
        timestamps = pd.date_range(start=f"{ev_id}-10-01 00:00:00", periods=n_hours, freq='h')
        
        # Distribuição de chuva por sub-bacia conforme a característica meteorológica de cada cheia
        if ev_id == '1983':
            # Chuva frontal contínua e persistente em toda a bacia
            p_total = {'oeste': 240.0, 'sul': 230.0, 'norte': 260.0, 'benedito': 210.0, 'mirim': 190.0, 'luis_alves': 160.0, 'acu': 220.0}
            peak_h = 36
        elif ev_id == '2008':
            # Chuva torrencial orográfica concentrada no Litoral, Baixo Vale e Médio Vale
            p_total = {'oeste': 110.0, 'sul': 90.0, 'norte': 180.0, 'benedito': 290.0, 'mirim': 340.0, 'luis_alves': 410.0, 'acu': 320.0}
            peak_h = 24
        elif ev_id == '2011':
            # Chuva concentrada no Alto e Médio Vale
            p_total = {'oeste': 210.0, 'sul': 195.0, 'norte': 230.0, 'benedito': 175.0, 'mirim': 160.0, 'luis_alves': 140.0, 'acu': 190.0}
            peak_h = 28
        else: # 2023
            # Dois pulsos sucessivos
            p_total = {'oeste': 190.0, 'sul': 180.0, 'norte': 200.0, 'benedito': 160.0, 'mirim': 170.0, 'luis_alves': 150.0, 'acu': 175.0}
            peak_h = 30

        rain_dict = {'timestamp': timestamps}
        for sb, tot in p_total.items():
            # Hietograma assimétrico com distribuição de chuva
            t_vec = np.arange(n_hours)
            sigma = 16.0
            hye = np.exp(-0.5 * ((t_vec - peak_h) / sigma)**2)
            if ev_id == '2023':
                # Adicionar segundo pulso
        ...
        # Séries observadas nas estações fluviométricas
        obs_hydro = {}
        st_peaks = {
        ...
        # Prazos típicos de onda observada para o evento
